...
int main(int argc, char** argv) {
  ...
  Body ibody[N/size], jbody[N/size], t[N/size];
  srand48(rank);
  ...
  for(int irank=0; irank<size; irank++) {
    ...
    MPI_Win win;
    MPI_Win_create(jbody, N*5*sizeof(double)/size, 5*sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win);
    for(int i = 0; i<N/size; i++) {
	     t[i] = jbody[i];
	  }
    MPI_Win_fence(0, win);
    MPI_Put(t, N/size, MPI_BODY, send_to, 0, N/size, MPI_BODY, win);
    MPI_Win_fence(0, win); 
    for(int i=0; i<N/size; i++) {
